...
void CMAC::SpoofMAC()
{
	...
	while (selection < range + 1)
	{
		...
		if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, _("SYSTEM\\CurrentControlSet\\Control\\Class\\{4D36E972-E325-11CE-BFC1-08002bE10318}"), 0, KEY_ALL_ACCESS, &hKey) == ERROR_SUCCESS)
		{
			...
			TCHAR Name[1024];
			DWORD cName = 1024;
			while (RegEnumKeyExA(hKey, dwIndex, (LPSTR)Name, &cName, NULL, NULL, NULL, NULL) == ERROR_SUCCESS)
			{
				HKEY hSubKey = NULL;
				if (RegOpenKeyExA(hKey, (LPSTR)Name, 0, KEY_ALL_ACCESS, &hSubKey) == ERROR_SUCCESS)
				{
					BYTE Data[1204];
					DWORD cbData = 1024;
					if (RegQueryValueExA(hSubKey, _("DriverDesc"), NULL, NULL, Data, &cbData) == ERROR_SUCCESS)
					{
						...
						if (_tcscmp((TCHAR*)Data, (const wchar_t*)wAdapterName) == 0)
						{
							...
							if (RegSetValueExA(hSubKey, _("NetworkAddress"), 0, REG_SZ, (const BYTE*)newMACAddr, sizeof(TCHAR) * ((DWORD)_tcslen((const wchar_t*)newMACAddr) + 1)) == ERROR_SUCCESS)
							{
